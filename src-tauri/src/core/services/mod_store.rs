...
mod tests {
    ...
    #[test]
    fn test_zip_entry_size_bounded() {
        ...
        // Write 1.5 MB of whitespace padding followed by valid JSON
        let padding = vec![b' '; 1_500_000];
        zip.write_all(&padding).unwrap();
        zip.write_all(b"{\"name\":\"oversized\",\"version\":\"1.0.0\"}").unwrap();
        zip.finish().unwrap();

        // Bounded read (1 MB max) cuts off before the JSON payload at 1.5 MB,
        // safely preventing memory exhaustion from oversized entry payloads.
        let err = read_info_json(&path).unwrap_err();
        assert!(err.to_string().contains("info.json is not valid JSON") || err.to_string().contains("EOF"));
