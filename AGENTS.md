...
- `components/packs/` — `PackCard`, `NewPackModal`. Other components: modals (`ModDetailsModal`, `DependencyPlanModal`, `VersionsModal`), `ModCard`, `QueueDrawer`, `Sidebar`.
...
- Styling: Tailwind 4, CSS-first tokens in `src/index.css` (`@theme`): `bg-app`/`bg-surface`/`bg-surface-2`, `border-line`, and a single swappable `accent` (green). Primary buttons are white by design; green is reserved for status/success. Icons: `lucide-react`. Font: Inter (bundled via `@fontsource-variable/inter`).
