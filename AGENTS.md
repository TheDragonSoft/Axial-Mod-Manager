...
| `packs.rs`        | Pack manifests stored as `profiles/<id>.json`. Import is tolerant (`format` accepted, not enforced). Sharing is Base64 text (`export_pack_base64`/`import_pack_base64` of the manifest JSON); legacy JSON file commands still exist. **Activation is target-state reconciliation**: diff target vs installed, download missing, finalize via `PendingActivation` state when downloads land, emit `pack-activated` and persist `Config.active_pack_id`. Built-in pseudo-pack `vanilla` = `activate_vanilla` (all mods disabled except `base`). |
